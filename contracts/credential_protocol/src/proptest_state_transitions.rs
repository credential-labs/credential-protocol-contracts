...
mod proptest_state_transitions {
    ...
    fn setup(env: &Env) -> CredentialProtocolContractClient<'_> {
    ...
    fn issue(client: &CredentialProtocolContractClient<'_>, env: &Env, issuer: &Address, subject: &Address) -> u64 {
        client.issue_credential(issuer, subject, &1u32, &meta(env, 8), &None, &0u64)
    }

    fn make_slice(client: &CredentialProtocolContractClient<'_>, env: &Env, creator: &Address, attestor: &Address) -> u64 {
        client.create_slice(
            creator,
