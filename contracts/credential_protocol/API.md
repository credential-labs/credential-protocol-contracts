# Credential Protocol Contract — API Reference

Credential Protocol is a Soroban smart contract that implements a **Federated Byzantine Agreement (FBA)**
...
### `verify_credential_holder(env, sbt_registry_id, zk_verifier_id, zk_admin, subject, credential_id, claim_type, proof, verifier) → bool`
...
| `subject`        | `Address`         | The credential holder being verified.                               |
...
### `verify_credential_holder_anonymous(env, zk_verifier_id, credential_id, claim_type, holder_commitment, proof) → bool`
...
### `verify_claim_batch(env, zk_verifier_id, zk_admin, credential_protocol_id, credential_id, claim_types, proofs) → Vec<bool>`
...
| `zk_verifier_id` | `Address`           | Address of the deployed ZK verifier contract.                |
| `zk_admin`       | `Address`           | Admin address forwarded to the ZK verifier.                  |
| `credential_protocol_id`| `Address`           | Address of this contract (forwarded to ZK verifier).         |
