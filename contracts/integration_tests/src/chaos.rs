...
// Chaos: credential revoked after SBT is minted — verify_credential_holder must not panic.
...
fn chaos_revoke_after_mint_graceful_degradation() {
    ...
    let result = c.qp.verify_credential_holder(
        &c.sbt.address,
        &c.zk.address,
        &c.admin,
        &engineer,
...
// Chaos: empty proof sent to verify_credential_holder — must return false, never panic.
#[test]
...
// Chaos: non-existent credential ID passed to verify_credential_holder — must return false.
#[test]
...
// Chaos: verify_credential_holder with mismatched credential ID — SBT belongs to cred N, not N+1.
#[test]
