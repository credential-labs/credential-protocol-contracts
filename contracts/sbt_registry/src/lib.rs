...
impl SbtRegistryContract {
    ...
    ///
    /// Cross-contract verifies via `credential_protocol` that the credential exists and is
    ...
    /// Panics if the credential does not exist or is revoked in `credential_protocol`.
    pub fn mint(env: Env, owner: Address, credential_id: u64, metadata_uri: Bytes) -> u64 {
        ...
        // Uses env.invoke_contract to avoid a circular crate dependency with credential_protocol.
        let qp_id: Address = env
            .storage()
            .instance()
            .get(&DataKey::CredentialProtocolId)
    ...
    /// Initialize the contract with an admin and the credential_protocol contract address.
    pub fn initialize(env: Env, admin: Address, credential_protocol_id: Address) {
        admin.require_auth();
        env.storage().instance().set(&DataKey::Admin, &admin);
        env.storage()
            .instance()
            .set(&DataKey::CredentialProtocolId, &credential_protocol_id);
    }
    ...
    /// Callable by the stored credential_protocol contract or the admin.
    ...
    pub fn batch_mint(env: Env, entries: Vec<BatchMintEntry>) -> Vec<u64> {
        ...
        // Fetch the CredentialProtocol contract address once.
        let qp_id: Address = env
            .storage()
            .instance()
            .get(&DataKey::CredentialProtocolId)
        ...
        for i in 0..entries.len() {
            ...
            // Requirement 1.3 / 1.4: verify credential is not revoked via CredentialProtocol.
            // is_revoked panics with CredentialNotFound if the credential doesn't exist.
    ...
    /// revocation (via the linked `credential_protocol` contract) before any entry is written.
    ///
...
mod tests {
    ...
    #[test]
    fn test_deploy_initialize_sets_admin_and_credential_protocol_id() {
        let env = Env::default();
        env.mock_all_auths();
        // Deploy a credential_protocol contract to use as the linked contract address.
    ...
    fn setup_with_qp(
        ...
        Address,
        CredentialProtocolContractClient,
        Address,
    ) {
        let qp_id = env.register_contract(None, CredentialProtocolContract);
        let qp_client = CredentialProtocolContractClient::new(env, &qp_id);
