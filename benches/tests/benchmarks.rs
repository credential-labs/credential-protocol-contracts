...
//   verify_credential_holder  : ~4_000_000 CPU / ~4_000_000 MEM  (cross-contract)
//   batch_issue (5)  : ~9_000_000 CPU / ~9_000_000 MEM
//   batch_verify (5) : ~3_000_000 CPU / ~3_000_000 MEM
//
// Run with: `cargo test -p credential-protocol-benches -- --nocapture`
use soroban_sdk::{testutils::Address as _, Address, Bytes, BytesN, Env, Vec};
use credential_protocol::{CredentialProtocolContract, CredentialProtocolContractClient};
...
const THRESHOLD_VERIFY_CREDENTIAL_HOLDER_CPU: u64     = 8_000_000;
...
const THRESHOLD_VERIFY_CREDENTIAL_HOLDER_MEM: u64     = 8_000_000;
...
fn setup_qp(env: &Env) -> (CredentialProtocolContractClient, Address) {
    env.mock_all_auths();
    let id = env.register_contract(None, CredentialProtocolContract);
    let client = CredentialProtocolContractClient::new(env, &id);
...
// ── credential_protocol benchmarks ───────────────────────────────────────────────────
...
fn bench_mint_sbt() {
    ...
    let qp_client = CredentialProtocolContractClient::new(&env, &qp_id);
    let admin = Address::generate(&env);
...
// ── Cross-contract: verify_credential_holder ──────────────────────────────────────────

/// Benchmarks the full cross-contract verify_credential_holder path:
/// CredentialProtocol → SbtRegistry (get_tokens_by_owner + get_token) → ZkVerifier (verify_claim).
/// This is the most expensive user-facing operation and the most regression-sensitive.
#[test]
fn bench_verify_credential_holder() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let m = measure(&env, || {
        qp_client.verify_credential_holder(
            &sbt_id,
            &zk_id,
            &admin,
            &engineer,
    ...
    println!("[bench_verify_credential_holder] cpu={} mem={}", m.cpu, m.mem);
    assert!(m.cpu <= THRESHOLD_VERIFY_CREDENTIAL_HOLDER_CPU,
        "verify_credential_holder CPU regression: {} > {}", m.cpu, THRESHOLD_VERIFY_CREDENTIAL_HOLDER_CPU);
    assert!(m.mem <= THRESHOLD_VERIFY_CREDENTIAL_HOLDER_MEM,
        "verify_credential_holder MEM regression: {} > {}", m.mem, THRESHOLD_VERIFY_CREDENTIAL_HOLDER_MEM);
}
