"""CredentialProtocol contract event exporter for Prometheus."""
...
class CredentialProtocolExporter:
    """Exports CredentialProtocol contract metrics to Prometheus."""
...
def main():
    ...
    contract_id = os.getenv("CONTRACT_CREDENTIAL_PROTOCOL")
    scrape_interval = int(os.getenv("SCRAPE_INTERVAL_SECONDS", "15"))
    exporter_port = int(os.getenv("EXPORTER_PORT", "9101"))

    if not contract_id:
        raise ValueError("CONTRACT_CREDENTIAL_PROTOCOL environment variable not set")

    exporter = CredentialProtocolExporter(
        rpc_url=rpc_url,
    ...
    logger.info(f"Starting CredentialProtocol exporter")
